...
class OutputNode extends Node{
  ...
  void removeReceiver(PVector nodeId){
    modules.get((int)nodeId.x).ins[(int)nodeId.y].flowId = -1;
    receivers.remove(receivers.indexOf(nodeId));
  }
  
  void clearReceivers(){
    for (int i = receivers.size()-1; i >= 0; i--){
      modules.get((int)receivers.get(i).x).ins[(int)receivers.get(i).y].flowId = -1;
      receivers.remove(i);
    }
  }  
}
...
class ModInput extends Node{
  ...
  //avoid the modifer setting the baseVal. Will need a better workaround.
  //maybe use pauseInput in conjunction with mouseEvents to only pause 
  //the controller being clicked.
