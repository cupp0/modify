...
class Modifier extends Module{
  ...
  Modifier(){
    ...
    modOuts[0] = new ModOutput(new PVector(id, 0, -1), new PVector(pos.x+size.x, pos.y+size.y));
  ...
  void operate(){
    ...
    float qStep = cp5.getController("modAux"+str(id)).getValue();
    modOuts[0].modData = new float[sampleLength];
    switch(type){
    case 0 : //sin
      for (int i = 0; i < sampleLength; i++){
        modOuts[0].modData[i] = amplitude*sin((float)i*2*PI/sampleLength);
      }
      break;
    case 1 : //square
      for (int i = 0; i < sampleLength; i++){
        if (i<sampleLength/2){
          modOuts[0].modData[i] = amplitude;
        } else {
          modOuts[0].modData[i] = -amplitude;
        }
      }
    ...
    case 2 : //saw
      for (int i = 0; i < sampleLength; i++){
        modOuts[0].modData[i] = amplitude-((float)i/sampleLength)*2*amplitude;
      }
      break;
    case 3 : //tri
      for (int i = 0; i < sampleLength; i++){
        modOuts[0].modData[i] = amplitude-abs(2*amplitude-((float)i/sampleLength)*4*amplitude);
      }
      break; 
    case 4 : //noise
      float seed = seeds.get(int(cp5.get(Textfield.class, "modNoiseSeed"+str(id)).getText()));
      for (int i = 0; i < sampleLength; i++){
        modOuts[0].modData[i] = map((float)noise.eval(seed+cos((float)i*2*PI/sampleLength), seed+sin((float)i*2*PI/sampleLength)), -1, 1, -amplitude, amplitude);
      }
      break; 
    case 5 : //random
      for (int i = 0; i < sampleLength; i++){
        modOuts[0].modData[i] = random(2*amplitude)-amplitude;
      }
      break;   
    }
    for (int i = 0; i < sampleLength; i++){
      modOuts[0].modData[i] = (1000*modOuts[0].modData[i]-1000*modOuts[0].modData[i]%qStep)/1000;
    }
    updateEnv();
  }
...
class Sampler extends Module{
  ...
  Sampler(){
    ...
    modOuts[0] = new ModOutput(new PVector(id, 0, -1), new PVector(pos.x+size.x-4, pos.y+size.y-4));   
